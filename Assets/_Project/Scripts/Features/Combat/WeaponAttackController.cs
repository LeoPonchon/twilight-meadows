...
public class WeaponAttackController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (sceneContext == null)
        {
            Debug.LogError("WeaponAttackController: Missing SceneContext in scene.", this);
            enabled = false;
            return;
        }
    ...
    private void PerformAttack()
    {
        ...
        int slot = inventoryManager != null ? inventoryManager.GetCurrentHotbarSlot() : 0;
        ItemStack stack = playerInventory.GetItemInSlot(slot);
        ToolKind toolKind = ItemInstanceFactory.GetToolKind(stack.itemData);
    ...
    private void ConsumeWeaponDurability(ItemStack stack)
    {
        if (stack.itemInstance is ToolInstance toolInstance)
        {
            ...
            // UI dÃ©couplÃ©e: la mise Ã  jour se fait via Events cÃ´tÃ© UI.
        }
    }
